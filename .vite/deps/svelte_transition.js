import { transition_slide_display, true_default } from "./chunk-7JI7KQHG.js";

// node_modules/.deno/svelte@5.51.2/node_modules/svelte/src/transition/index.js
var linear = (x) => x;
...
function slide(
    ...
    const style = getComputedStyle(node);
    if (
        true_default && !slide_warning &&
        /(contents|inline|table)/.test(style.display)
    ) {
        slide_warning = true;
    ...
    const secondary_properties = axis === "y"
        ? ["top", "bottom"]
        : ["left", "right"];
    ...
    return {
        ...
        css: (t) =>
            ...
            }: ${t * padding_start_value}px;padding-${
                secondary_properties[1]
            }: ${t * padding_end_value}px;margin-${secondary_properties[0]}: ${
                t * margin_start_value
            }px;margin-${secondary_properties[1]}: ${
                t * margin_end_value
            }px;border-${secondary_properties[0]}-width: ${
                t * border_width_start_value
            }px;border-${secondary_properties[1]}-width: ${
                t * border_width_end_value
            }px;min-${primary_property}: 0`,
    };
}
function scale(
    node,
...
function draw(
    node,
    { delay = 0, speed, duration, easing = cubic_in_out } = {},
) {
