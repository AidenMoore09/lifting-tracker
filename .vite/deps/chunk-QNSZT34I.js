...
function reconcile(state2, array, anchor, flags, get_key) {
	...
	if (is_animated) {
		for (i = 0; i < length; i += 1) {
			...
			effect2 = /** @type {EachItem} */ items.get(key2).e;
...
function animation(element2, get_fn, get_params) {
	var effect2 = animation_effect_override ?? /** @type {Effect} */ active_effect;
	var nodes =
		/** @type {EffectNodes} */
		effect2.nodes;
	var from;
...
function head(hash2, render_fn) {
	...
	if (hydrating) {
		...
		while (
			head_anchor !== null &&
			(head_anchor.nodeType !== COMMENT_NODE || /** @type {Comment} */ head_anchor.data !== hash2)
		) {
...
function selectedcontent(element2, update_element) {
	...
	attach(element2, () => () => {
		...
		const observer = new MutationObserver((entries) => {
			...
			if (selected) {
				element2.replaceWith((element2 = /** @type {HTMLElement} */ element2.cloneNode(true)));
...
function createEventDispatcher() {
	...
	return (type, detail, options) => {
		const events =
			/** @type {Record<string, Function | Function[]>} */
			/** @type {string} */
			active_component_context.s.$$events?.[type];
		if (events) {
