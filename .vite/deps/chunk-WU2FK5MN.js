...
import { async_mode_flag, legacy_mode_flag, tracing_mode_flag } from './chunk-LJXMJ73G.js';
...
function setContext(key, context) {
	...
	if (async_mode_flag) {
		...
		var valid =
			!active_reaction &&
			(flags2 & BRANCH_EFFECT) !== 0 && // pop() runs synchronously, so this indicates we're setting context after an await
...
var Batch = class _Batch {
	...
	#traverse_effect_tree(root, effects, render_effects) {
		...
		while (effect2 !== null) {
			...
			var skip =
				is_skippable_branch || (flags2 & INERT) !== 0 || this.#skipped_branches.has(effect2);
			if (
				async_mode_flag &&
				pending_boundary === null &&
				(flags2 & BOUNDARY_EFFECT) !== 0 &&
...
function pause_children(effect2, transitions, local) {
	...
	while (child2 !== null) {
		...
		var transparent =
			(child2.f & EFFECT_TRANSPARENT) !== 0 || // If this is a branch effect without a block effect parent,
...
function merge_text_nodes(text) {
	...
	while (next2 !== null && next2.nodeType === TEXT_NODE) {
		next2.remove();
		text.nodeValue += /** @type {string} */ next2.nodeValue;
